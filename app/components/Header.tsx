    ...
    return(
        <div id="header-container" className="flex w-full h-12 justify-between items-center px-4">
            <div className="flex flex-1">
                <Link className='pr-4 text-2xl w-35 h-5 font-medium flex items-center' href={window.location.origin}>
                    BuyMyStuff
                </Link>
                <SearchBar />
            </div>
            <div className="flex flex-1 justify-between">
                <ul className='flex pl-20 font-medium gap-10'>
                    <Link href={`${window.location.origin}/viewProducts?category=all&sort=asc_name`}>Shop All</Link>
                    <li>Shop By Category</li>
                    <Link href={`${window.location.origin}/viewProducts?category=all&sort=best_sellers`}>Best Sellers</Link>
                    <Link href={'/viewProducts?category=all&sort=highest_discount'}>Sale</Link>
                </ul>
                <div className='flex gap-6'>
                    <Icon path={mdiAccount} color={"black"} size={0} />
                    <Link href={`${window.location.origin}/cart`} className='relative'>
